...
PROGRAM_ID="ELvgA3nDX1oSTsCaw5AaGZvuHoWTfpNR3nTRpFPM48ar"
WALLET="${ANCHOR_WALLET:-$HOME/.config/solana/id.json}"
PROGRAM_KEYPAIR="target/deploy/anchor_escrow-keypair.json"
MIN_SOL="2.0"
...
if anchor idl fetch "$PROGRAM_ID" --provider.cluster devnet >/dev/null 2>&1; then
  anchor idl upgrade --filepath target/idl/anchor_escrow.json "$PROGRAM_ID" --provider.cluster devnet
else
  anchor idl init --filepath target/idl/anchor_escrow.json "$PROGRAM_ID" --provider.cluster devnet
fi
